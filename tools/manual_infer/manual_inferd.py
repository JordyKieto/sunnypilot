...
def main():
  ...
  model = ModelState()
  main_wide_camera, use_extra_client = _main_stream()
  ...
  pm = messaging.PubMaster(["modelV2", "drivingModelData", "cameraOdometry", "modelDataV2SP"])
  ...
  try:
    while params.get_bool("ManualInferMode"):
      ...
      if model_output is not None:
        ...
        payload["policy_outputs"] = {k: (v.tolist() if hasattr(v, "tolist") else v) for k, v in model_output.items()}
        payload["action"] = {
      ...
      _write_status(step, "running", payload)
      step += 1
